...
class OpenAI_Client(BaseClient):
    ...
    def _call(self, messages: str, **kwargs):
        if "seed" not in kwargs and 'gemini' not in self.model:
        ...
        num_retries = kwargs.pop("num_retries", 6)
        ...
        for i in range(num_retries):
            try:
                ...
                if "stream" in kwargs and kwargs["stream"]:
                    if return_dict:
                        raise NotImplementedError("Streaming not supported with return_dict=True")
                    r = ""
                    reasoning_content = ""
                    for chunk in response:
                        if chunk.choices[0].delta.content is not None:
                            r += chunk.choices[0].delta.content
                        # Capture reasoning content for reasoning models
                        if hasattr(chunk.choices[0].delta, 'reasoning') and chunk.choices[0].delta.reasoning is not None:
                            reasoning_content += chunk.choices[0].delta.reasoning
                    
                    # If we have reasoning content, return it in a structured way
                    if reasoning_content.strip():
                        r = f"<think>\n{reasoning_content.strip()}\n</think>\n\n{r}"
                else:
                    r = response.choices[0].message.content
                    # For non-streaming, check if reasoning is available
                ...
                if post_check_function is None:
                    ...
                    if return_dict:
                        post_r = post_check_function(response)
                    else:
                ...
                logger.error("Error in LLM Client: ")
                logger.error(traceback.format_exc())  # This will print the full stack trace
                # Exponential backoff with jitter for transient errors (e.g., 429, gateway, JSON decode)
                sleep_s = min(60, (2 ** i)) + 0.1 * i
                time.sleep(sleep_s)
